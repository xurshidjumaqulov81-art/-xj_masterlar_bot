...
from texts import *
from database import get_stats, set_registration_status, set_registration_limit, get_all_users, search_user, delete_user_by_id, get_all_telegram_ids, add_admin, remove_admin

router = Router()

def user_line(user):
    user_id, full_name, xj_id, phone, gender, region, confirm_code, telegram_id, username, created_at = user
    uname = f"@{username}" if username and username != "Йўқ" else "Йўқ"
...
async def set_limit(message: Message, state: FSMContext):
    if not message.text.isdigit() or int(message.text) <= 0:
        await message.answer("❌ Фақат мусбат рақам киритинг.")
        return
    await set_registration_limit(int(message.text))
    await message.answer(SET_LIMIT_DONE_TEXT.format(limit=int(message.text)), reply_markup=admin_keyboard)
    await state.clear()
...
async def list_users(message: Message):
    ...
    if not users:
        await message.answer("Ҳали рўйхатдан ўтганлар йўқ.", reply_markup=admin_keyboard)
        return
    chunks = []
    text = "👥 Рўйхатдан ўтганлар:\n\n"
    for user in users:
        part = user_line(user) + "\n"
        if len(text) + len(part) > 3900:
    ...
    if len(chunks) > 3:
        await message.answer("Рўйхат жуда узун. Қидиришдан фойдаланинг.")
...
async def send_broadcast(message: Message, state: FSMContext, bot: Bot):
    sent = failed = 0
    for telegram_id in await get_all_telegram_ids():
        try:
            await bot.send_message(telegram_id, message.text)
            sent += 1
        except Exception:
            failed += 1
    await message.answer(BROADCAST_DONE_TEXT.format(sent=sent, failed=failed), reply_markup=admin_keyboard)
    await state.clear()
...
async def search_user_handler(message: Message, state: FSMContext):
    ...
    if not results:
        ...
        text = "🔍 Қидирув натижалари:\n\n" + "\n".join(user_line(u) for u in results[:20])
...
@router.message(AdminFilter(), AdminStates.delete)
async def delete_by_number(message: Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("❌ Фақат № рақамини киритинг.")
        return
    ok = await delete_user_by_id(int(message.text))
    await message.answer(USER_DELETED_TEXT if ok else NOT_FOUND_TEXT, reply_markup=admin_keyboard)
    await state.clear()
...
async def add_admin_handler(message: Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("❌ Telegram ID фақат рақам бўлиши керак."); return
